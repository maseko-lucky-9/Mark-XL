...
Verifies that when enable_loopguard=False:
  ...
  - The 188 pre-WO-5+WO-6 baseline tests remain unaffected (marker test; WO-6 additive +18).
...
class TestAC6FlagOffByteEquality:
    ...
    def test_baseline_count_marker(self):
        """AC-6: the pre-WO-5+WO-6 baseline (non-WO-5 tests) is unchanged at 188.
        ...
        assert len(baseline_nodeids) == 188, (
            "Pre-WO-5+WO-6 baseline must remain EXACTLY 188 tests (additive-only "
